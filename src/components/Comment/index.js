...
const Comment = ({comment, blog, onUpdateComment, onDeleteComment}) => {
    ...
    return (
        <div className="mt-3 d-flex">
            <img src={comment.user?.avatar  && `${process.env.REACT_APP_URL_BE}/images/image/${comment.user.avatar.filename}`} width={35} height={35} alt="user-avatar"/>
            <div className="ms-2 content-comment">
                        ...
                        <>
                            <div className="input-edit-comment mt-1 d-flex justify-content-between align-items-center">
                                ...
                                {contentComment !== comment.content ?
                                    <i className="fas fa-save" onClick={() => {onUpdateComment(comment._id, contentComment); setEditComment(false)}}></i>
                                    :   
                ...
                <div className="text-secondary mt-1" style={{"fontSize": "12px"}}>
                    {(userInfo?.slug === blog?.user.slug || userInfo?.slug === comment.user.slug) ?
                        <span className="action-comment" onClick={() => onDeleteComment(comment._id)}>Xóa</span>
