...
import { actSetLogin, processErrResponse } from "../../redux/actions/browser.action";
...
const UpdateUser = () => {
    ...
    const validateSchema = yup.object().shape({
        ...
        phone: yup.string(),
        address: yup.string().min(10, 'Từ 10 đến 200 ký tự').max(200, 'Từ 10 đến 200 ký tự'),
        country: yup.string(),
    })
    // use hook form
    ...
    const onSubmit = async (data) => {
        try{
            ...
            localStorage.setItem('token-yb', resultUpdate.data.data.token);
            ...
            <div className="update-user d-flex">
                ...
                <div class="image-upload">
                    <label for="file-input">
                        <img src={userInfo?.avatar  && `${process.env.REACT_APP_URL_BE}/images/image/${userInfo?.avatar}`} alt="user-avatar"data-bs-toggle="tooltip" data-bs-placement="right" title="Đổi avatar" onChange={(e) => onSubmitChangeAvatar(e.target.value)}></img>
                    </label>

                    <input id="file-input" type="file" />
                </div>

                {/* <img src={userInfo?.avatar  && `${process.env.REACT_APP_URL_BE}/images/image/${userInfo?.avatar}`} alt="user-avatar"></img> */}
